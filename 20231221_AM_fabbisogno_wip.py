...
tabella_fabbisogni = pvt_2
tabella_fabbisogni.reset_index(inplace=True)
tabella_fabbisogni.Data = tabella_fabbisogni.Data.astype('datetime64[ns]')

st.write ('Tabella fabbisogni', tabella_fabbisogni)
...
#df_raw['Ordine'] = df_raw['Ordine'].astype('Int64')
#st.write('Fino qui', df_raw)

df_max = df_raw[['Ordine','Dt.eff.inizio esec.','Operazione']] # appena aggiunto, verificare se ok con originale
df_max = df_max.groupby(['Ordine']).max()

#df_max = df_raw.groupby(['Ordine']).max() modifica
df_max.reset_index(inplace=True)
#df_max = df_max[['Ordine','Dt.eff.inizio esec.','Operazione']]  modifica
